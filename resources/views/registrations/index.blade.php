...
<body>
    ...
    <div class="p-4 sm:ml-48">
        ...
        <a class="px-3 py-2 text-sm text-white bg-blue-700 hover:bg-blue-800 focus:ring-2 focus:ring-blue-300 font-medium rounded-lg focus:outline-none"
            role="button" href="{{ route('registrations.create') }}">tambah pesanan</a>
                            ...
                            <tr class="bg-white border-b hover:bg-gray-50 ">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <form onsubmit="return confirm('Hapus &#34;{{ $registration->nama }}&#34;');"
                                        ...
                                        method="POST">
                                        <div class="">
                                            <a class="px-3 py-2 text-sm focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-2 focus:ring-green-300 font-medium rounded-lg"
                                                role="button"
                                                href="https://api.whatsapp.com/send?phone={{ $registration->no_hp }}&text={{ urlencode('Hai Kak ' . $registration->nama . ', kami dari RecHouse studio ingin memastikan apakah Kakak berencana untuk hadir pada ' . \Carbon\Carbon::parse($registration->waktu_booking)->isoFormat('dddd, D MMMM YYYY HH:mm:ss') . '? Apakah Kakak akan datang pada waktu itu atau mungkin ingin mengatur ulang jadwal?') }}"
                                                target="_blank" rel="noopener noreferer">kirim
                                                pesan</a>
                                            <a class="px-3 py-2 text-sm text-white bg-blue-700 hover:bg-blue-800 focus:ring-2 focus:ring-blue-300 font-medium rounded-lg focus:outline-none"
                                                role="button"
                                                href="{{ route('registrations.edit', $registration->id) }}">edit</a>
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
